...
import com.ondmoney.sn.history.IntegrationTest;
...
class HistoryServiceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/history-service-kafka/publish?message=value-produce").with(csrf())).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/history-service-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/history-service-kafka/unregister"));
                return;
            }
        }
